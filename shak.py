...
class DynamicConverterApp(QWidget):
    ...
    def create_add_subtract_panel(self):
        ...
        # Add k input for result digits
        k_layout = QVBoxLayout()
        k_label = QLabel("Result Configuration:")
        self.k_input = QLineEdit()
        self.k_input.setPlaceholderText("Number of digits for result (k)")
    ...
    # Updated calculation method for Add/Subtract
    def calculate_add_sub(self):
        try:
            ...
            # Get k value for result digits
            if not self.k_input.text():
                raise ValueError("Please enter number of digits for result (k)")
            ...
            # Perform calculation using the provided functions
            if self.add_radio.isChecked():
                result = b_sum(num1_str, num2_str, k, d01, d02)
                operation = "+"
            else:
                result = subtraction(num1_str, num2_str, k, d01, d02)
                operation = "-"
            
            # Display result
            self.add_sub_result.setText(
                f"Result: {num1_str} (d={d01}) {operation} {num2_str} (d={d02}) = {result}"
            )
